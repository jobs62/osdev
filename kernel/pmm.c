...
void dump_bitmap() {
    for (unsigned int index = 0; index < PAGE_LEN / 2; index += 4) {
        kprintf("0x%8h 0x%8h 0x%8h 0x%8h\n", 
                ((unsigned int *)bitmap)[index], 
                ((unsigned int *)bitmap)[index+1],
                ((unsigned int *)bitmap)[index+2], 
                ((unsigned int *)bitmap)[index+3]);
    }
}