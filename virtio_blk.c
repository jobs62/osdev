...
static int virtio_blk_read(void *bdev, uint32_t numsect, uint32_t lba, void *edi) {
    ...
    kprintf("head 0x%8h; data: 0x%8h; status 0x%8h\n", get_physaddr(&header), get_physaddr(edi), get_physaddr(&status));
...
void virtio_blk_init(struct pci_header *head, uint8_t bus, uint8_t slot, uint8_t fonc) {
    ...
    device.queue.desc = (struct virtq_desc *)add_vm_entry((void *)0xC0200000, totan_queue_size, VM_MAP_ANONYMOUS | VM_MAP_WRITE | VM_MAP_KERNEL, (void *)0);
