...
static inline int getexp( unsigned int size )
{
	...
	{
		#ifdef DEBUG
		kprintf("getexp returns -1 for %1d less than MINEXP\n", size);
		#endif
	...
	#ifdef DEBUG
	kprintf("getexp returns %1d (%1di bytes) for %1di size\n", shift - 1, (1<<(shift -1)), size );
	#endif
...
static void dump_array()
{
	...
	kprintf("System memory allocated: %1d\n", l_allocated );
	kprintf("Memory in used (malloc'ed): %1d\n", l_inuse );

		for ( i = 0; i < MAXEXP; i++ )
		{
			kprintf("%2d(%1d): ",i, l_completePages[i] );
	
			tag = l_freePages[ i ];
			while ( tag != NULL )
			{
				if ( tag->split_left  != NULL  ) kprintf("*");
				kprintf("%1d", tag->real_size );
...
static struct boundary_tag* allocate_new_tag( unsigned int size )
{
	...
	struct boundary_tag *tag;
		...
		#ifdef DEBUG
		kprintf("Resource allocated 0x%0h of %1d pages (%1d bytes) for %1d size.\n", tag, pages, pages * l_pageSize, size );

		l_allocated += pages * l_pageSize;

		kprintf("Total memory usage = %1d KB\n",  (int)((l_allocated / (1024))) );
		#endif
...
void *malloc(size_t size)
{
	...
	liballoc_lock();
		...
		if ( ((int)(remainder) > 0) /*&& ( (tag->real_size - remainder) >= (1<<MINEXP))*/ )
		{
			...
			{
				#ifdef DEBUG
				kprintf("Seems to be splittable: %1d >= 2^%1d .. %1d\n", remainder, childIndex, (1<<childIndex) );
				#endif
				...
				#ifdef DEBUG
				kprintf("Old tag has become %1d bytes, new tag is now %1d bytes (%1d exp)\n", tag->real_size, new_tag->real_size, new_tag->index );
				#endif
			}	
		}
...
void free(void *ptr)
{
	...
	liballoc_lock();
		...
		{	
			...
			if ( l_completePages[ index ] == MAXCOMPLETE )
			{
				...
				#ifdef DEBUG
				l_allocated -= pages * l_pageSize;
				kprintf("Resource freeing 0x%8h of %1d pages\n", tag, pages );
				dump_array();
				#endif
	...
	#ifdef DEBUG
	kprintf("Returning tag with %1d bytes (requested %1d bytes), which has exponent: %1d\n", tag->real_size, tag->size, index ); 
	dump_array();
	#endif
